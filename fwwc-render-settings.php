...
HHHH;
  ...
  $gateway_valid_for_use = FWWC__is_gateway_valid_for_use($gateway_status_message);
  if (!$gateway_valid_for_use)
  {
    ...
    "Faircoin Payment Gateway is NOT operational (try to re-enter and save settings): " . $gateway_status_message .
    '</p>';
  }
  else
  {
    ...
    "Faircoin Payment Gateway is operational" .
    '</p>';
  }
  ...
  if (!$currency_code || $currency_code=='FAIR')
    $currency_code = 'USD';

  $exchange_rate_message =
    '<p style="border:1px solid #DDD;padding:5px 10px;background-color:#cceeff;">' .
    FWWC__get_exchange_rate_per_faircoin ($currency_code, 'getfirst', true) .
    '</p>';
...
<!--
        <tr valign="top">
          <th scope="row">Faircoin Service Provider:</th>
          <td>
            <select name="service_provider" class="select ">
              <option <?php if ($fwwc_settings['service_provider'] == 'electrum_wallet') echo 'selected="selected"'; ?> value="electrum_wallet">Your own Electrum wallet</option>
              <option <?php if ($fwwc_settings['service_provider'] == 'blockchain_info') echo 'selected="selected"'; ?> value="blockchain_info">Blockchain.info API (deprecated - use Electrum instead)</option>
            </select>
            <p class="description">
              Please select your Faircoin service provider and press [Save changes]. Then fill-in necessary details and press [Save changes] again.
...
-->
        <tr valign="top">
          ...
          <td>
            <textarea style="width:75%;" name="electrum_mpk_saved"><?php echo $fwwc_settings['electrum_mpk_saved']; ?></textarea>
            <p class="description">
        ...
        <tr valign="top">
          ...
          <td>
            <input type="text" name="confs_num" value="<?php echo $fwwc_settings['confs_num']; ?>" size="4" />
            <p class="description">
              After a transaction is broadcast to the Faircoin network, it may be included in a block that is published
...
<!--
        <tr valign="top">
          <th scope="row">Faircoin Exchange rate calculation type:</th>
          <td>
            <select name="exchange_rate_type" class="select ">
              <option <?php if ($fwwc_settings['exchange_rate_type'] == 'vwap') echo 'selected="selected"'; ?> value="vwap">Weighted Average</option>
              <option <?php if ($fwwc_settings['exchange_rate_type'] == 'realtime') echo 'selected="selected"'; ?> value="realtime">Real Time</option>
              <option <?php if ($fwwc_settings['exchange_rate_type'] == 'bestrate') echo 'selected="selected"'; ?> value="bestrate">Most profitable</option>
            </select>
            <p class="description">
...
-->
        <tr valign="top">
          ...
          <td>
            <input type="text" name="exchange_multiplier" value="<?php echo $fwwc_settings['exchange_multiplier']; ?>" size="4" />
            <p class="description">
              Extra multiplier to apply to convert store default currency to faircoin price.
              <br />Example: 1.05 - will add extra 5% to the total price in faircoins.
              May be useful to compensate for market volatility or for merchant's loss to fees when converting faircoins to local currency,
              or to encourage customer to use faircoins for purchases (by setting multiplier to < 1.00 values).
            </p>
          </td>
        </tr>

        <tr valign="top">
            <th scope="row">Reuse expired addresses?</th>
            <td>
              <input type="hidden" name="reuse_expired_addresses" value="0" /><input type="checkbox" name="reuse_expired_addresses" value="0" <?php if ($fwwc_settings['reuse_expired_addresses']) echo 'checked="checked"'; ?> />
              <p class="description">
                <b>Yes</b> (default, recommended) - will allow to recycle faircoin addresses that been generated for each placed order but never received any payments. The drawback of this approach is that potential snoop can generate many fake (never paid for) orders to discover sequence of faircoin addresses that belongs to the wallet of this store and then track down sales through blockchain analysis. The advantage of this option is that it very efficiently reuses empty (zero-balance) faircoin addresses within the wallet, allowing only 1 sale per address without growing the wallet size (Electrum "gap" value).
                <br />
                ...
                Extreme privacy mode offers the best anonymity and privacy to the store albeit with the drawbacks of potentially flooding your Electrum wallet with expired and zero-balance addresses. Hence, for vast majority of cases (where you just need a secure way to operate faircoin based store) it is suggested to set this option to 'Yes').<br />
        ...
        <tr valign="top">
          ...
          <td>
            <input type="hidden" name="autocomplete_paid_orders" value="0" /><input type="checkbox" name="autocomplete_paid_orders" value="1" <?php if ($fwwc_settings['autocomplete_paid_orders']) echo 'checked="checked"'; ?> />
        ...
        <tr valign="top">
            ...
            <td>
              ...
              <p class="description">
                ...
                <br /><b style="color:red;">NOTE:</b> Cron jobs <b>might not work</b> if your site is password protected with HTTP Basic auth or other methods. This will result in WooCommerce store not seeing received payments (even though funds will arrive correctly to your faircoin addresses).
                ...
                <!-- <br/>For secure, fast hosting service optimized for wordpress and 100% compatibility with WooCommerce and Faircoin payments we recommend <b><a href="http://hostrum.com/" target="_blank">Hostrum Hosting</a></b>. -->
              </p>
            </td>
        </tr>
...
<p style="text-align:center;"><?php echo FWWC__GetProLabel(); ?></p>
<p><h3>Advanced Settings section gives you many more options to configure and optimize all aspects and functionality of your faircoin store.</h3>
  Please note that if you are among many who made a donation toward the development of FaircoinWay software - you will receive an equivalent credit toward the
  <a href="<?php echo FWWC__GetProUrl(); ?>"><b>Pro version</b></a>.
</p>
<h3 style="text-align:center;color:#090;">Get the <a href="<?php echo FWWC__GetProUrl(); ?>"><b>PRO version</b></a></h3>


      </table>

      <p class="submit">
          <input type="submit" class="button-primary"    name="button_update_fwwc_settings"        value="{$value_save_changes}"             />
          <input type="submit" class="button-secondary"  style="color:red;" name="button_reset_partial_fwwc_settings" value="{$value_reset_settings}" onClick="return confirm('Are you sure you want to reset settings on this page?');" />
      </p>
  </form>
TTT;
  echo $html_out;
