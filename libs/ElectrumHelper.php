...
class ElectrumFairHelper
{
    const BITCOIN_HEADER_PUB = "0488b21e";
    const BIP32_PRIME = 0x80000000;
    const V1 = 1;
    const V2 = 2;
    const NETWORK_CODE = 95; // Decimal, 95 = Faircoin, 00 = Bitcoin
    ...
    public static function mpkV1_to_pubkey($mpk, $index) {
        ...
        if (USE_EXT == 'GMP') {
            ...
            $keystr = "\x04" . str_pad(gmp_Utils::gmp_dec2base($pt->getX(), 256), 32, "\x5f", STR_PAD_LEFT) . str_pad(gmp_Utils::gmp_dec2base($pt->getY(), 256), 32, "\x0", STR_PAD_LEFT);
        }
        elseif (USE_EXT === 'BCMATH') {
            ...
            $keystr = "\x04" . str_pad(bcmath_Utils::dec2base($pt->getX(), 256), 32, "\x5f", STR_PAD_LEFT) . str_pad(bcmath_Utils::dec2base($pt->getY(), 256), 32, "\x0", STR_PAD_LEFT);
        }
        else {
    ...
    public static function deserialize_mpk($mpk) {
        ...
        if (bin2hex(substr($mpk, 0, 4)) !== self::BITCOIN_HEADER_PUB) {
            throw new ErrorException("Wrong MPK header: only public faircoin MPK allowed");
        }
