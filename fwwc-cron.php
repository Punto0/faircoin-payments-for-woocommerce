...
function FWWC_cron_job_worker ($hardcron=false)
{
  ...
  $confirmations_required = $fwwc_settings['confs_num'];
  ...
  if (is_array($rows_for_balance_check))
  {
  	...
  	foreach ($rows_for_balance_check as $row_for_balance_check)
  	{
		  ...
		  $balance_info_array = FWWC__getreceivedbyaddress_info ($address_request_array, $fwwc_settings);
        ...
        if ($balance_info_array['balance'] > 0)
        {
          ...
          FWWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['fair_address']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");

          if ($balance_info_array['balance'] < $last_order_info['order_total'])
          {
            FWWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: balance at address: '{$row_for_balance_check['fair_address']}' (FAIR '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
          }
        }
        else
        {
        ...
        //	Scan '$address_meta['orders']' for first UNPAID order that is exactly matching amount at address.
	        ...
	        FWWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['fair_address']}' (FAIR '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
	        ...
	        FWWC__process_payment_completed_for_order ($last_order_info['order_id'], $balance_info_array['balance']);
...
// 	        if (!$hardcron && $ran_cycles >= $fwwc_settings['soft_cron_max_loops_per_run'])
// 	        {
...
// 	        }
		  ...
		  {
		    FWWC__log_event (__FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['fair_address']}: " . $balance_info_array['message']);
		  }
		  //..//
		}
	}
